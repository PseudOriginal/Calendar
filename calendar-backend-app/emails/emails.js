...
var DailyPostman = module.exports = {
	...
	getTodayDates: function() {
		const tmp = new Date()
		const todayStart = new Date(tmp.getFullYear(), tmp.getMonth(), tmp.getDate(), 0, 30, 0)
		tmp.setDate(tmp.getDate() + 1)
		const todayEnd = new Date(tmp.getFullYear(), tmp.getMonth(), tmp.getDate(), 0, 29, 59)
	...
	setDailyReminder: async function(email) {
		this.dailyReminders[email] = cron.schedule('*/5 * * * *', () => {
			...
			events.forEach(event => {
				const now = new Date()
				const diffMins = Math.round((((event.startDate - now) % 86400000) % 3600000) / 60000)
